...
export async function getFinishgoodHariIni() {
  ...
  const response = await fetch(
    `${API_BASE_URL}/barang/hari-ini?status=${status}&limit=${limit}`,
  );
  ...
  const fallbackResponse = await fetch(
    `${API_BASE_URL}/barang?tanggal=${date.year}-${date.month}-${date.day}` +
      `&status=${status}&limit=${limit}`,
  );

  if (!fallbackResponse.ok) {
    throw new Error(
      `API error: ${fallbackResponse.status} ${fallbackResponse.statusText}`,
    );
  }

  return fallbackResponse.json() as Promise<{
    tanggal?: string;
    data?: unknown[];
    meta?: { total?: number };
  }>;
