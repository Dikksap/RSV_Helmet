...
export function BarangTable({
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-brand-border bg-brand-surface-card shadow-sm">
          ...
          <tbody className="divide-y divide-brand-border/60 text-sm">
            {barang.map((item, index) => (
              ...
              >
                ...
                <td className="px-4 py-2.5 text-xs text-brand-grey-light">
      ...
      <div className="flex items-center justify-between border-t border-brand-border bg-brand-surface/40 px-4 py-2 text-xs text-brand-grey">
        <span>
          Menampilkan {barang.length} dari {totalBarang} barang
        </span>
        <span>
          {totalBarang > 0 ? `${(currentPage - 1) * 20 + 1}–${Math.min(currentPage * 20, totalBarang)}` : "0"}
        </span>
      </div>
    </div>
  );
